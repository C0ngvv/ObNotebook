...
**使用未修改的生成器**。近似于fuzzstruction的方法，我们可以使用afl++向生成器提供突变的输入，并将生成器产生的输出馈送给消费者。除了fuzzstruction之外，发电机本身在这里没有动过。这有许多缺点:首先，许多应用程序不使用任何输入(genrsaμ、gendsaμ、mke2fs或signμ)。其次，其他应用程序，如7zip(μ)或zip只将输入包装在压缩容器中。fuzzstruction的方法可以修改包装器本身，即产生轻微损坏的压缩文件，这是afl++突变的输入数据无法实现的。第三，对于不受这些缺陷影响的应用程序，例如qpdfμ或pngtopng，使用afl++产生输入仍然受到afl++无法克服更复杂约束的影响。例如，qpdfμ需要一个PDF文件
