...
- 规则生成器由1500行Python代码实现。我们使用Simhash算法[31]来检测近重复项，并使用 Stanford dependency parser[51]来建立依赖树。
...
## 9.结论
为了了解网络犯罪分子如何发起与物联网相关的攻击，我们利用蜜罐来收集现实世界物联网漏洞的痕迹，并分析了四种流行的攻击工具包。我们的研究揭示了近年来物联网攻击普遍存在的一个被忽视的原因:物联网漏洞是公开可用的，很容易被利用，而今天的物联网攻击几乎完全使用已知的漏洞来进行恶意攻击。更重要的是，我们的研究结果导致了IoTShield的设计，这是一个简单而有效的用于入侵检测的物联网漏洞特定签名生成系统。IoTShield首先收集过去20年的43万份漏洞报告，识别7500份物联网漏洞报告的内容。然后，IoTShield检索关键知识以生成特定于漏洞的签名。这些签名可以很容易地部署在现有的入侵检测系统或web应用程序防火墙中，以检测对目标物联网设备的攻击企图。因此，IoTShield能够显著提高未来物联网攻击成功的门槛。
