...
GDBFuzz依赖于被触发的错误是可观察的，这意味着错误或其他错误行为必须由错误触发。因此，除非在编译过程中使用额外的杀毒器，否则无法发现[39]中所演示的静默破坏。例如，可以通过发生连接错误(如超时或错误响应代码)来检测故障。此外，可以通过调试接口在故障处理程序的位置上设置断点，该接口还可以捕获来自已部署的杀毒程序的故障信号。
