...
**进程感知观察**。有了识别客户机中当前正在执行的进程的能力，我们就可以开始观察具有进程感知的与线程相关的系统调用执行情况。最初，我们过滤与文件系统相关的系统调用，这些调用不会影响文件状态或修改文件内容(例如，read)。然后，我们根据其参数将其余与文件相关的系统调用分为两种类型，如表1所示:一种类型是直接处理文件路径;另一种类型是处理文件描述符。对于后一种类型，我们不容易在主机上获得相应的文件描述符。因此，我们提出用进程感知观察文件关系，如图2所示。首先，我们监视创建文件的系统调用(例如，open)并获取返回的文件描述符$fd_{guest}$ 。此外，我们还标识执行系统调用的当前执行进程p。与此同时，我们在主机上重新执行相同的系统调用，并获得返回的文件描述符$fd_{host}$。由于固件可以通过执行类似于dup的系统调用来创建文件描述符的副本，因此我们将相关的客户机文件描述符添加到集合$FD_{guest}$中，如公式2所示。由于Linux是一个多任务操作系统，子进程可以继承文件描述符来参与对文件的修改。因此，我们将进程关联起来，并将它们添加到P，如公式3所示。之后，我们创建映射M，如公式4所示，它建立了P、$FD_{guest}$和$fd_{host}$之间的关系。这意味着，在随后的执行中，如果进程属于P，并且它在属于$FD_{guest}$的文件描述符上执行其他与文件相关的系统调用，那么我们将在主机上重新执行$fd_{host}$上的系统调用。

![](images/Pasted%20image%2020230420092822.png)

重播。对于在guest机器中调用的每个与文件相关的系统调用，我们在主机机器上重新执行它以同步文件系统状态。一般来说，我们在算法1中展示了文件系统调用执行重播的整个过程。算法的输入包括(1)$id_{sys}$：我们正在重放的系统调用；(2)$p^*$ ：在来guest上执行系统调用$id_{sys}$的进程; (3)$obj\_arg$ ：$id_{sys}$的参数，它指向文件对象。它可以是文件路径或文件描述符；(4)$other\_arg$ ：$id_{sys}$需要的除了$obj\_arg$的其他参数；(5) RDIR：解压后的固件文件系统在主机上的绝对目录路径。

![](images/Pasted%20image%2020230420093326.png)

与文件相关的系统调用执行的重放过程可以分为两个部分。对于处理文件路径$path_{guest}$的系统调用，我们推断一个新的文件路径$path_{host}$，以确保重播可以限制在提取的文件系统目录中，而不是主机上的根目录中。如果$path_{guest}$是一个绝对路径,我们在$path_{guest}$的前面添加RDIR来生成$path_{host}$。如果$path_{guest}$是一个符号链接和源头$path_{source}$是一个绝对路径,我们在$path_{source}$添加RDIR。在此之后，我们在$path_{host}$上执行系统调用(第2行12)。另一方面，对于处理文件描述符的与文件相关的系统调用，我们标识当前正在执行的进程$p^*$，并利用通过公式4构建的映射M。在此基础上，我们找到了$fd^*_{host}$，并最终在主机上对它执行系统调用(第14行)。
...
进程资源限制是指应用程序可以使用的特定内核资源的最大数量，资源限制设置不当可能会影响PUT的行为。例如，资源限制RLIMIT_NOFILE指定PUT可以打开的文件描述符的最大数量。在一些put的主执行阶段，它们可能遍历所有系统描述符，其数量范围从0到RLIMIT_NOFILE限制。因此，设置较大的RLIMIT_NOFILE限制值会减慢PUT的执行速度。实际上，由于系统性能的不同，主机内核中的RLIMIT_NOFILE值要比嵌入式固件中的RLIMIT_NOFILE值高得多。因此，我们不能直接使用主机上的资源限制值。

因此，我们利用观察-重放策略为用户模式模拟中的PUT设置进程资源限制。请注意，Linux内核提供了setrlimit和getrlimit系统调用来设置或获取进程资源限制的值。因此，我们通过在固件全系统仿真期间提前监控getlimit来检索进程资源限制值。之后，当PUT第一次通过getlimit请求资源限制时，我们直接向它们提供观测值。
...
## 6. 评估
在QEMU[38]的基础上，对全系统代码和用户模式代码进行了修改，实现了EQUAFL的原型。此外，我们还修改AFL以改变AFL -fuzz接受的参数，从而正确加载目标应用程序的参数和环境变量。

通过原型机，我们从以下几个方面对EQUAFL的性能进行了评估。

- **兼容性**。首先，我们对EQUAFL进行了评估，验证了EQUAFL是否能够成功地对AFL无法直接模糊的应用进行模糊处理。此外，我们将EQUAFL中目标应用程序的系统调用序列/轨迹与基于全系统仿真的模糊分析进行比较，以检查我们增强的用户模式仿真(土耳其6.2)的正确性。
- **效率**。我们首先通过与纯用户模式仿真进行比较来测量系统中引入的开销。然后，我们通过比较全系统仿真AFL和Firm-AFL的吞吐量来评估EQUAFL的效率。
- **漏洞的发现**。我们评估EQUAFL是否可以在真实世界的嵌入式fjrmware应用程序中成功发现漏洞。

### 6.1 实验设置
**基准**。在我们的实验中，我们使用了两个不同的数据集作为基准：第一个数据集包含两组标准基准:nbench[30]和lmbench[31]。另一个数据集由70个嵌入式固件镜像组成，来自三大嵌入式设备供应商，包括D-Link、TRENDnet和NETGEAR。最初，我们通过抓取相关供应商的网站来收集固件映像。然后，我们在AFL-Full和Firm-AFL上运行它们，得到了可以成功仿真和测试的固件样本。最终，我们获得70个不同产品系列的固件样本作为第二个数据集的候选。表2列出了固件映像的汇总信息。

![](images/Pasted%20image%2020230413205523.png)

标准数据集中的程序更小，不包含错误。因此，我们用它们来评价EQUAFL的兼容性和效率。我们从真实世界的固件中收集的应用程序包含bug，因此我们不仅使用它们来评估兼容性和效率，而且还演示EQUAFL的效率提升如何帮助发现真实世界的bug。

**基线**。我们在实验中选择了三条基线。

•AFL-User。AFL- user遵循默认的AFL设置，其中使用用户模式QEMU作为模拟器。

•AFL-Full。我们将AFL与QEMU的全系统仿真相结合，以支持全系统仿真中目标应用程序的模糊化。在系统中，我们利用DECAF[20]中的VMI模块来监控目标应用程序的进程。

•Firm-AFL。Firm-AFL是一款最先进的灰盒模糊器，利用全系统和用户模式模拟来支持基于linux的物联网应用程序的高效模糊。

**配置**。为了提高所有实验的模糊处理效率，我们在AFL中使用了字典选项。通过对目标应用程序的静态分析来收集用作字典标记的关键字。对于每个目标应用程序，我们提供一个正常的网络请求作为初始种子。

为了进行兼容性和效率评估，实验在12核Intel(R) Xeon(R) E5-1650 v3 3.50GHz CPU机上进行，操作系统为Ubuntu 18.04.5 LTS，内存为15.6GB。用于漏洞发现评估的机器为18核Intel(R) Xeon(R) CPU E5-2699 v3 2.30GHz CPU机器，Ubuntu 18.04.5 LTS LTS操作系统，188GB RAM。

为了减轻模糊器的随机性，每个实验重复5次。此外，我们还根据[24]的建议进行了Mann-Whitney U测试[33]，并计算出了A12值[42]。

### 6.2 兼容性
表3显示了EQUAFL和AFLUser在真实数据集上的执行结果。根据Firmadyne的说明选择实际应用程序。AFL-Full和Firm-AFL都可以成功地模拟所有这些参数。我们将这些结果分为四种状态:(1)CRA-当fuzzer尝试使用初始种子进行试运行时，应用程序崩溃;(2)HAN-当fuzzer尝试使用初始种子进行试运行时，应用程序挂起;(3)ERR-应用程序可以由fuzzer启动。但是，fuzzer报告了一些错误，例如当它尝试使用初始种子进行试运行时没有找到文件。(4)SUCC-应用程序进程可以由fuzzer启动，没有显式地报告错误。

![](images/Pasted%20image%2020230413210137.png)



### 6.4 漏洞发现
我们将EQUAFL应用于真实的应用程序，以揭示它们的漏洞。如表5所示，EQUAFL共能发现10个漏洞，影响18个嵌入式设备系列。然后手动分析了这些漏洞的根源，最终确认9个是NULL指针解引用漏洞，1个是整数overfmow漏洞。我们也在网上搜索漏洞信息，之前只有一个漏洞被其他安全研究人员发现。据我们所知，其余9个漏洞仍然不为公众所知。我们已经向厂家进行了汇报，其中6个已经确认为零日，并分配了CVE编号。

我们在有bug的实际应用程序上进行了24小时的实验，比较EQUAFL、Firm-AFL和AFL-Full的模糊结果。首先，我们为每个漏洞选择一个固件应用程序。对于有多个漏洞的固件映像，我们仍然选择一个测试应用程序。然后，我们以5次重复并行进行每个模糊实验，持续24小时。

图6显示了EQUAFL、Firm-AFL和AFL-Full在一段时间内发现的独特漏洞的平均数量。图6中的线条是检测到的唯一漏洞的平均数量。结果表明，EQUAFL可以比Firm-AFL和AFL-Full更快地发现所有漏洞(除了EQUAFL和Firm-AFL并列的一个漏洞)。表4显示了每种技术暴露第一个漏洞的时间。uTTE测量5次运行的平均时间。因子改进(factor)衡量uTTE与基线相比的性能增益。A12意味着如果我们随机从5次重复中选取一个运行，EQUAFL在多大概率下可以比基线表现得更好。我们可以看到EQUAFL发现第一个漏洞的速度明显快于AFL-Full和Firm-AFL。

![](images/Pasted%20image%2020230413205105.png)

> 总之，EQUAFL可以比AFL-Full和Firm-AFL更快地检测到漏洞。这表明增强的EQUAFL用户模式仿真确实有助于提高灰盒模糊器的漏洞检测能力。
