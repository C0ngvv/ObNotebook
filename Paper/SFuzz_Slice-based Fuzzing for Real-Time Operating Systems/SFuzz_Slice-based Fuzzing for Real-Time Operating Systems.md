...
### 5.1 真实世界漏洞
SFuzz在20个不同设备的固件样本中发现了77个新bug，包括路由器、打印机、防火墙和BCI。截止提交时，68个漏洞已被厂商确认，67个漏洞已被分配CVE或CNVD ID (CVE 46个，CNVD 21个，重度漏洞64个);9个bug仍在等待供应商的回复。图3显示了与这些错误相对应的数据接收点和代码片段的任务类型。
