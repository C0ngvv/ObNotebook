...
最后，EmTaint执行基于sse的别名分析以及间接调用目标푡푝푟푡与原始引用푓푝푡푟/푑푝푡푟之间的和依赖关系。图5演示了间接调用解析的过程。EmTaint反向跟踪푡푝푟푡到及其所有别名。同时，EmTaint 通过正向和反向分析，找到了푓푝푡푟和푑푝푡푟的别名。然后，EmTaint收集每个函数的入口块和出口块中的所有别名sse，检查푓푝푡푟/푑푝푡푟的别名与푡푝푟푡的别名之间的数据依赖关系，并进行匹配和更新。我们将푡푝푟푡的别名sse表示为퐶푇푒푥푝푟(调用目标表达式)，将푓푝푡푟或푑푝푡푟的别名sse表示为푃푒푥푝푟(指针表达式)。接下来，我们解释如何分析퐶푇푒푥푝푟和푃푒푥푝푟，并推断与函数表相关的间接调用目标。

![](images/Pasted%20image%2020231018204219.png)
