...
![](images/Pasted%20image%2020231019083939.png)
...
定义1 (AAG)。AAG是一个有向属性图G(N,E,S,Ftran,Btran,Fprop,Bprop)，其中N是节点集{n1，n2，…，nk}，每个节点ni表示一条指令，E是一组有向边，表示节点之间的控制转移关系，每个节点有四组独立的SSE集作为节点属性，S是每个节点上所有SSE集的并集。最后，Ftran和Btran是两组传递函数，分别在前向分析和后向分析期间更新每个节点上的SSE集。Fprop和Bprop分别是沿着或反着边的方向传播SSE的两组函数。
...
- $Fprop_{n\rightarrow m}$：对于一个边$n \rightarrow m$，它传播节点n的fout中的SSE到节点m的fin集合。
- $Bprop_{n\rightarrow m}$：对于一个边$n \rightarrow m$，它传播节点m的bout中的SSE到节点n的bin集合。

定义2 (AAG-Q)。AAG-Q是一种基于AAG的按需别名分析方法。给定特定函数f的AAG和节点m上的指针p作为查询，AAG-Q运行以下算法在该函数AAG的每个节点上输出p的潜在别名:

1) 初始化节点m的fin集合作为{p}
2) 运行一个多迭代进程，直到满足以下终止条件:所有节点上的fin集和bin集不再随着额外的迭代而增长。需要注意的是，当满足终止条件时，将到达一个固定点fixed point。
3) 每次迭代包括两个pass:前向分析pass和后向分析pass。
4) 在每次迭代中，前向分析传递的工作原理如下:
- 首先，通过对函数f的图AAG进行反向后置遍历来访问每个节点。
- 当访问节点n时，如果该节点既不是调用点节点，也不是函数的开始/返回节点，则执行以下操作:
$$
\begin{align}
	&fout_n,bout_n = Ftran_n(fin_n,n) \\
	&Fprop_{n \rightarrow s}: fin_s = fin_s \bigcup fout_n, s \in succ_n \\
	&Bprop_{r\rightarrow n}: bin_r = bin_r \bigcup bout_n, r \in pred_n, \\
	&s\in succ_n表示AAG图中s是节点n的后继，r \in pred_n 表示r是节点n的前继
\end{align}
$$
- 如果当前的节点是一个调用点节点，
- 如果当前的节点是一个起始节点，它执行前向操作除了Bprop。
- 如果当前的节点是一个返回节点，
5) 在每次迭代过程中，反向分析过程的工作原理如下:
- 首先，通过对函数f的图AAG的后置遍历来访问每个节点。
- 访问节点n时，如果该节点既不是调用点节点，也不是函数的开始/返回节点，则执行以下操作:
$$
\begin{align}
& fout-n, bout_n = Btran_n(bin_n, n) \\
& Fprop_{n\rightarrow s}:fin_s = fin_s \bigcup fout_n, s\in succ_n \\
& Bprop_{r \rightarrow n}: bin_r = bin_r \bigcup bout_n, r\in pred_n \\
\end{align}
$$
- 如果当前节点是调用点节点，则
- 如果当前节点是起始节点，
- 如果当前节点是返回节点，它执行上述操作，除了Fprop。

我们算法的第二方面。我们的算法进行过程间按需别名分析，但这并不意味着我们的算法将始终积极地处理(目标程序中的)每个函数。实际上，许多函数在被触发之前将保持“休眠”状态，并且当满足以下任何条件时，将触发函数的别名分析。(a)查询将自动触发使用指针p的函数的别名分析。该函数是第一个将被我们的算法分析的函数。此时，所有其他函数实际上都处于“休眠”状态。(b)当函数A的别名分析结束时，如果A有调用者函数，则触发调用者函数的分析。当条件(a)满足时，我们算法的Aspect 1描述了我们算法执行的所有操作。但是，当满足条件(b)时，我们的算法在Aspect 1之外执行以下操作。首先，它检查调用者函数的AAG-Q中返回节点的fin集和调用点节点的前继节点的bin集是否发生了变化。然后，如果它发生变化，调用方函数将作为Aspect 1进行分析。否则，调用函数将回到“休眠”状态。

#### 3.2.3 传递函数内更新规则
在上一节中，我们介绍了过程间按需别名分析的迭代算法，该算法涉及到更新每个正常节点上SSE集的传递函数(Ftran和Btran)，在本节中，我们详细说明了相应的更新规则。在每个正常节点上，Ftran和Btran的规则如表2所示。

![](images/Pasted%20image%2020231019093859.png)

#### 3.2.4 处理数据流循环和递归数据结构
描述内存访问变量的SSE是一种无存储模式[8]，它代表了一个无界的值域，这可能会导致我们的算法无法到达fixed点。为了解决这个问题，EmTaint使用k-limiting[16]方法来限制SSE的长度，其中k设置为5。在k限制下，我们的算法的输入集fin和bin保证了收敛性，我们的算法保证了到达一个fixed点。除了k-limiting方法外，EmTaint还使用访问路径抽象来抽象数据循环和递归数据结构。
