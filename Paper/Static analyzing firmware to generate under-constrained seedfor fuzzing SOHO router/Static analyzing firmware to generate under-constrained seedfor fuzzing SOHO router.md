...
**两阶段模糊测试**。我们观察到后端操作句柄分为两类。receiver用于从前端获取数据并对其进行处理(例如POST请求)，而returner用于响应前端请求并返回数据(例如get请求)。为了尽可能正确地测试所有操作句柄，我们在receiver之后发送一个返回器并重复循环。我们使用一个简单的启发式策略来区分接收者和返回者。在所标识的所有操作句柄中，使用key-value函数获取网络数据的操作句柄是接收者，而其余的操作句柄被认为是返回者。我们将对固件中与web相关的文件的请求也视为返回者。作为一种模糊测试方法，在无法确定HTTP请求类型的情况下，UCRF使用突变(例如，预定义的GET或POST类型)填充HTTP请求方法。