...
### 4.2 实验设置
在实验中，我们从五个不同的供应商中选择了10个设备进行测试。表2显示了它们的信息。表格的最后一列显示了每个设备所支持的获取植入基于信号的监视器权限的方法。

除了基于信号的监视器，SRFuzzer的所有模块都部署在Ubuntu 16.04上。它通过电缆和Mi Smart插头通过稳定的无线连接连接路由器。SRFuzzer对每个设备连续模糊40小时。在模糊测试过程中，如果电源控制模块在超过6分钟的时间内没有收到任何响应，它将重新启动设备。

![](images/Pasted%20image%2020231010201333.png)

### 4.3 整体实验
我们总共监控了208个独特的异常行为。在Mutator模块添加的额外信息的帮助下，我们可以确认101个独特的问题并手动完成PoCs。它们的详细信息显示在表3的最后一列中。

根据负责任的披露政策向相关供应商报告后，101个独特问题中有97个已得到供应商的确认，另有4个问题正在评估过程中。这97个漏洞被分配了官方ID，包括43个CVE ID、52个PSV ID和2个CNVD ID。

![](images/Pasted%20image%2020231010201630.png)

**确认问题**。对于每个设备，SRFuzzer收集的种子数量列在第2列，上述每种类型(即内存损坏的MEM，命令注入的CMD，跨站点脚本的XSS和信息泄露的INFO)的问题数量按两个触发阶段(即CONF和READ)分组在第3-8列。在101个已确认的问题中，内存损坏问题48个(47.52%)，命令注入问题39个(38.61%)，XSS问题9个(8.91%)，信息泄露问题5个(4.95%)。CONF操作触发的问题占67.33%，READ操作触发的问题占32.67%。除TL-WVR900G外，确认存在十余个问题的5台设备均发现了多种类型的漏洞。通过对其实现的分析，我们发现其后端采用Lua语言实现，避免了内存损坏。

此外，我们从他们的CVSS版本3分数[1]中评估了97个正式确认的漏洞的影响及其影响，如图7所示。我们使用基于CVSS分数的官方四个排名来显示漏洞的严重性。具体而言，超过四分之一(25/97)的问题处于高水平。我们还根据其影响将问题数量分为四类，即特权升级(EoP)、脚本执行、拒绝服务(DoS)和信息泄露。大多数问题属于EoP类别，因为我们可以制作它们的poc来劫持控制流以执行低级系统的命令，因此我们将web管理特权升级为root特权。几乎所有poc都可以通过一条消息危及目标设备。

![](images/Pasted%20image%2020231010203150.png)

**监视器的效果**。表4显示了不同监控器捕获的不同已确认问题的分布情况。我们可以看到，大多数已确认的问题(77.23%)被基于响应的监控器和基于代理的监控器捕获，显示了这些与设备无关的监控器的有效性。我们还发现基于信号的监视器捕获了另外23个问题，显示其发现深度内存损坏漏洞的能力。需要注意的是，基于信号的监视器可以捕获WNDR-4500v3和R7800的更多问题。这是由于它们的特殊实现造成的。在它们的后端，它们创建子进程来处理请求，并且在子进程崩溃时总是响应“配置失败”。在这种情况下，基于信号的监视器(而不是基于响应的监视器)可以处理它们。

![](images/Pasted%20image%2020231010203206.png)

图8显示了SRFuzzer报告的独特异常行为的数量，与我们确认的问题进行了比较。在这个实验中，我们独立运行每个监视器。因此，不同的监视器捕捉到了一些已确认的问题，如图8所示。平均而言，我们的监视器的准确率为48.56%。除信息泄露外，各类漏洞的准确率均在40.00%以上，非常有前景。

![](images/Pasted%20image%2020231010203304.png)

**重启**。图9给出了在模糊测试期间每个设备的重启次数。在我们测试的40小时内，每台设备平均重启6.8次。有趣的是，R8500的重启次数是R7800的3.6倍。这是因为R8500只在一个进程中处理所有请求，而R7800通过创建子进程来处理请求。此外，基于openwrt[18]操作系统的设备(如TL-WVR900G)比其他设备更稳定。

![](images/Pasted%20image%2020231010203344.png)

### 4.4 比较
我们比较了SRFuzzer和三个流行的开源fuzzer在发现漏洞方面。对于内存损坏漏洞，我们选择boofuzz[27]作为比较工具，boofuzz是著名协议fuzzer Sulley[21]的分支和继承者。对于命令注入漏洞，我们选择了Commix[37]而不是boofuzz，因为Commix具有更多的突变规则和监控方法。对于跨站攻击漏洞，我们选择了wfuzz[28]，这是一种流行的支持跨站攻击检测的web模糊测试工具。SRFuzzer在信息泄露漏洞方面没有与其他进行比较，因为大多数结果(5个中的4个)正在评估中。我们将在以后的工作中进行比较。

在比较中，在4家厂商中选择了7款设备。在这些设备上，我们连续40小时不间断地运行所有工具，这与SRFuzzer相同。我们向他们提供原始请求，这与SRFuzzer相同。为了满足boofuzz的特殊输入要求，我们将每个具有k-v对的种子转换为boofuzz的数据表示。如图10所示，SRFuzzer在所有类型的漏洞中都优于这三个比较fuzzer。具体来说，它发现的内存损坏问题比boofuzz多53.57%，命令注入问题比Commix多25.81%。同时，SRFuzzer比wfuzz多发现了一个XSS问题。我们详细分析了结果。

![](images/Pasted%20image%2020231010203647.png)

**内存损坏**。Boofuzz无法使用其默认数据表示找到任何漏洞。我们在boofuzz的数据表示中编码由k-v对组成的种子。通过这种方式，boofuzz可以有效地改变请求内容字段，从而触发漏洞。但是，由于缺乏多个监视方法，boofuzz可能会错过READ操作和子进程中发生的问题。

**命令注入**。由于两个原因，Commix的真正阳性率低于SRFuzzer。首先，在其众多的监测方法中，“时间相关”注射监测技术适合于设备。然而，该技术依赖于响应时间，这使得Commix错过了一些短期异常。其次，Commix只监控CONF操作过程中的异常，忽略了READ操作中出现的问题，例如AC9和TL-WVR900G中的问题。

**跨站点脚本(XSS)**。SRFuzzer比wfuzz多发现了一个XSS问题。它是R8500。由于没有K-V模型的引导，生成的输入无法绕过后端有效性检查，因此wfuzz忽略了它。因此，不能通过CONF操作成功地存储精心制作的值，并且返回到前端的值不会触发XSS问题。

## 5.讨论
在本节中，我们讨论了当前模糊框架的局限性，并探讨了未来的改进方向。

**范围的限制**。管理协议满足C-R模型和K-V模型的物联网设备可以利用SRFuzzer。我们将扩展我们的工作，以应用更多类型的设备，如摄像机、交换机和打印机，以及其他广泛使用的管理协议，如SOAP。

**严重的脆弱性**。虽然SRFuzzer可以在没有认证的情况下模糊设备，但在我们的实验中，它没有发现任何预先认证的问题。因此，我们将更彻底地检查攻击面，找到隐藏的接口，从而增强发现认证绕过漏洞的能力。

**数据不一致的研究**。基于C-R模型和K-V模型，我们注意到特定K-V对的不同后端过程之间存在一些数据不一致。在本文中，我们专注于自动模糊过程，并将语义相关性的系统分析留给未来的研究。深入的研究还可以帮助供应商巩固他们的安全设计。

**监控**。从监控的角度来看，SRFuzzer利用了两个方面的优势:针对不同类型漏洞的各种监视器和针对同一类型漏洞的各种监控机制。我们将尝试改进侵入式监视器的通用性，例如，提出一个框架来重新打包固件或支持直接闪存写入。

## 6.相关工作
物联网设备漏洞检测的研究很多，也可以应用于SOHO路由器。A.Costin等人[13]对固件映像进行了大规模分析，但在运行时没有发现任何问题。H.Bojinov等[7]审计了几种类型的嵌入式管理接口，B.Gourdin等[24]提出了WebDroid来构建安全的嵌入式web接口。对于SOHO装置也提出了类似的工作[25]，[17]。另一种大规模漏洞检测是扫描互联网上的旧设备。A.Cui[14]提出了全球范围内易受攻击嵌入式设备数量的定量下限。他们发现超过54万个嵌入式设备配置了出厂默认的根密码。

模糊测试是一种自动发现漏洞的有效方法。反馈驱动的fuzzers[9]、[22]、[23]、[31]、[43]、[33]使用运行时代码覆盖率来指导以下输入的生成。You [41]， Jain[26]研究了输入字节的自动输入类型推断。模糊过程可能会在复杂条件下的特定分支上产生粘滞。为了解决这一问题，研究者将模糊与符号执行相结合[10]，[30]，[38]。虽然它们很有前途，但由于缺乏内部运行时信息，因此不适合路由器。

模拟设备是获取运行时信息的一种潜在解决方案。A.Costin等[12]使用qemu来模拟某些基于linux的设备的web界面。该方法不适用于包含特殊硬件相关操作(如Wi-Fi配置)的web界面。FIRMADYNE[15]通过支持设备的NVRAM仿真来仿真基于linux的COTS固件。但在我们的实践中，它仅限于基于ARM的设备。

为了克服固件获取和仿真问题，提出了对物理设备进行模糊测试。Z.Wang等[40]开发了RPFuzzer来模糊SNMP等路由器协议。IoTFuzzer[11]是一个基于应用程序的模糊测试框架，旨在发现物理物联网设备中没有固件映像的内存损坏。它利用了模糊测试和污染分析的协同作用。它只专注于移动到web的界面，并且仅通过活动性检查检测内存损坏，这不足以发现web服务器漏洞。此外，M.Muench[32]分析了对嵌入式设备进行模糊测试的挑战，并提出了六种检测内存损坏的启发式方法。

程序分析技术也用于物联网设备发现漏洞。Q.Feng等[19]采用基于图的方法搜索固件映像中的漏洞。他们将控制流图转换为数字特征向量，并使用了几种哈希技术来实现实时搜索。Y.Shoshitaishvili等[36]采用静态符号执行和程序切片的方法寻找后门。此外，还使用了动态符号执行。FIE[16]是用于查找MSP430固件bug的符号执行框架。Avatar[42]是在分析固件时协调仿真器和设备的框架。SRFuzzer是对这些技术的补充。

## 7.结论
我们提出了SRFuzzer在没有设备仿真的全自动模式下识别SOHO路由器的多类型漏洞。为了更全面、更深入地发现漏洞，我们设计了KEY-VALUE数据模型和CONF-READ通信模型来揭示数据不一致性，指导模糊测试。通过6条突变规则和3种监测机制的协同，我们测试了10台物理路由器，并在责任披露后分配了43个CVE id、52个PSV id和2个CNVD id。