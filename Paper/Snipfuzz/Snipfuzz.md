...
为了建立一个有效的模糊测试，如图2所示，Snipuzz首先执行一个代码片段确定过程。具体来说，snizz在一个种子푆∧퐶中选择一条消息푚，从中生成一条探测消息푝푖和相应的序列푀푖。푀푖中的每个消息将触发响应消息푟푖(简称响应)，其中包含有关执行输出的信息。Snipuzz为每条消息푚分配一个响应池푅，该响应池用于确定新响应푟푖是否唯一。响应的唯一性表示该响应不属于响应池中存在的任何响应类别。如果푟푖是唯一的，Snipuzz将把푟푖添加到池푅中，并保留相应的消息序列푀푖作为新的种子。然后Snipuzz根据响应将消息푚分成不同的片段(第4.2节)。在获得片段后，Snipuzz根据各种策略执行突变，例如，空，字节翻转，数据边界或浩劫(详见4.3节)。在整个模糊过程中，Snipuzz设置了一个网络监视器来检测可能表明漏洞的崩溃(第4.4节)。
