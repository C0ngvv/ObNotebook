...
### 5.2 关系识别的准确性(RQ2)
确定显式声明的关系的准确性。在主动学习过程中，我们进行了70次迭代，收集了1817个人工标记的数据作为训练集。我们使用最终模型来预测数据集中的所有句子(除了训练集和验证集)，其中7,483个句子被预测为积极的，221,344个句子被预测为消极的。我们从每个类别中随机抽取500个句子，并手动标记以供评估。该模型在评价数据集上的准确率为92.90%，具有较高的准确率。假阳性率和召回率分别为11.49%和98.42%，这意味着我们的方法将r -句识别为非r -句的风险较低，从而可以避免遗漏一些关系。

然后，我们在20个程序手册页的句子上评估我们的模型。根据人工标注的结果，2952个句子中有75个句子是显式r句。我们的模型预测了76个句子是肯定的，包括9个假阳性和8个假阴性。准确率为98.80%，假阳性率为0.67%，召回率为89.33%。虽然该数据集的准确性更高，但其F1得分(0.89)低于之前的评价数据集(0.92)。主要原因可能是这个数据集是不平衡的。请注意，在关系提取过程中，9个假阳性中有8个被判断为中立并被忽略(见第3.4节)，这并不影响提取结果。

识别隐式声明冲突的准确性。从这些文档中，我们发现隐式声明了232个唯一冲突对。我们使用3.2节的方法进行鉴定，共发现218对冲突对，其中假阳性9对，假阴性23对。准确率为95.87%，召回率为90.09%，表明我们的方法在识别隐式声明冲突方面具有良好的性能。经过我们的分析，这九个误报都是由于缺乏相关的常识。例如，tiffcrop中-X和-Y选项的主题句是“设置要提取的区域相对于指定的原始引用的水平/垂直维度”。虽然这两个选项都是“设置尺寸”，但是我们的工具并不知道水平和垂直尺寸是不同的，这需要人类的常识。7次假阴性也是由于缺乏相关常识。在jpegoptim中-s选项的主题句“从输出文件中剥离所有标记”中，我们的工具不知道“所有标记”指的是所有类型的标记，而不是特定的标记。因此，它忽略了s选项和选项之间隐式声明的冲突，以剥离单一类型的标记。七个假阴性来自我们实现的NLP解析器的不准确解析结果(例如，SpaCy)。例如，在两个结构相同的主题句“output monochrome sixel image”和“output 15bpp sixel image”中，解析器将“monochrome”判断为名词，而将“15bpp”判断为数字，导致这两个句子被误认为是不平行的，从而被识别为不冲突的。9个假否定是由于主题句的细微差异造成的。例如，-pvk-strong和-pvk-none选项的主题句是“Enable Strong PVK encoding level”和“Don 't forced PVK encoding”。由于动词(enable vs . enforcement)和对象(level vs . encoding)不同，我们的工具无法从这些句子中识别隐式声明的冲突。有趣的是，我们发现这种缺失的冲突可以从他们的名字中识别出来。

### 5.3 关系提取精度(RQ3)
在手工标注后，我们从20个程序中发现了305个关系，其中包括284个冲突和22个依赖关系。我们的工具发现了282种关系，其中264种是冲突，18种是依赖。冲突的查准率和查全率分别为95.83%和89.40%，依赖的查准率分别为100%和81.82%。所有关系的查准率和查全率分别为96.10%和88.85%，表明我们的工具在关系提取方面表现良好。我们发现依赖关系可能更难提取，导致召回率较低，精度较高。在依赖性的四个假阴性中，两个是由于模型的假阴性，另外两个是由于无法找到相应的选项。例如，我们的模型识别了pdftotext中的r句子，“这在所有其他模式中被忽略”，但是我们的工具无法找到与“所有其他模式”对应的选项。事实上，即使是人类也很难找到与这些“模式”相对应的选项。例如，只有当我们将-layout选项的名称与-table选项的描述“Table模式类似于物理布局模式”结合起来时，我们才能发现-layout选项对应于物理布局模式。在冲突的11个假阳性和30个假阴性中，分别有5个和32个是由于未能识别r句和隐式声明冲突(见5.2)。两个是由于无法找到相应的选择。另外两起是由于缺乏人类常识而导致的错误提取。-RSAPublicKey_in和-RSAPublicKey_out选项放在一起，共享描述“类似于-pubin和-pubout，只是使用了RSAPublicKey格式。”由于描述的主题不是复数，我们的工具错误地将这两个选项视为相同的选项，导致假阳性(即-RSAPublicKey_in与-pubout)和假阴性(即-RSAPublicKey_out与-pubout)。

### 5.4 优先排序技术的有效性(RQ4)
为了评估我们在第3.5节中提到的优先化技术的有效性，我们从我们的真实数据集中随机选择了10个程序，并为每个程序提供了5个经过重组的有效组合列表(CarpetFuzzRand)，以与优先化的组合进行比较。每个组合列表被模糊处理48小时。然后我们以唯一路径的平均个数n¯作为评价标准，定义为:

![](images/Pasted%20image%2020230704230114.png)

其中Eb在5.1节中表示基线(AFL、AFLfast、MOPT和afl++)在五次运行中的边缘覆盖的联合，代表它们的覆盖能力。Efi表示CarpetFuzz和CarpetFuzzRand在第i次运行中的边缘覆盖率。结果如表2所示。通过我们的优先级排序技术，CarpetFuzz在每个程序上发现了其他fuzzers无法发现的更多边缘(平均多7%)，证明该技术在发现更多独特边缘方面是有效的。在三个项目中，改善更为明显(超过10%)。特别是在openssl-ec中，CarpetFuzz发现的独特边缘比CarpetFuzzRAND多20%。在7个程序中，发现独特边缘的能力略有提高(在5%以内)。

在分析那些具有最低优先级的组合时，我们发现一些冲突仅在运行时声明。例如，在img2sixel中有四种冲突(-P与-8、-P与-e、-P与-I以及-P与-b)没有在任何在线文档、手册或帮助命令中声明。只有在使用这些组合中的任何一种运行程序时，我们才能得到提示，例如“选项-p， -colors与-I， -high-color冲突”。我们还发现，tiffcrop中的两个依赖项在运行时被声明为冲突(-K对-S， -J对-S)。我们可以在源代码中找到作者留下的注释，“也许有一天，但不是现在”，这意味着文档中的依赖项存在，但尚未实现。尽管这些无效的组合在前面的步骤中被忽略了，我们的优先级划分技术仍然通过对演练的覆盖进行排序来发现它们，并给予它们最低的优先级。

### 5.5 与最先进技术相比的模糊处理性能(RQ5)
